...
@section('content')
    <main id="main" class="main">
        ...
        <hr>
        <div class="card">
            <form method="post" action="{{ route('varision.update', $varision->id) }}" encvarision="multipart/form-data"
                class="row g-3 p-3">
                @csrf
                @method('PUT')
                <div class="col-md-12 pb-3">
                    <label for="name" class="form-label">Varision Name<span class="text-danger">*</span></label>
                    <input type="text" class="form-control" id="name" name="name" value="{{ $varision->name }}"
                        required>
                    @error('name')
